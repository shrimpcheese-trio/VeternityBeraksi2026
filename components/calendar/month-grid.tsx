  ...
  return (
    <div className="rounded-2xl border border-border bg-background p-5">
      <div className="mb-4 flex items-center justify-between">
        <a
          href={`/worker/calendar?month=${prevMonth}`}
        ...
        <a
          href={`/worker/calendar?month=${nextMonth}`}
      ...
      <div className="grid grid-cols-7 gap-1">
        ...
        {weeks.map((week, wi) =>
          week.map((day, di) => {
            ...
            const hasProof = proofDateSet.has(dateStr);

            return (
              <a
                key={dateStr}
                href={`/worker/calendar?month=${thisMonth}&day=${dateStr}`}
                className={cn(
