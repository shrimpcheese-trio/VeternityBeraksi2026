    ...
    <div>
      <form action={action}>
        <div className="space-y-5">
          ...
          <div className="flex items-center justify-between pt-1">
            ...
            <a href="/forgot-password" className="text-sm font-bold text-sky hover:text-navy transition-colors">
              {t("forgotPassword")}
            </a>
          </div>

          {state.error && <p className="text-sm text-red-500 font-medium bg-red-50 p-3 rounded-lg border border-red-100">{state.error}</p>}

          <Button type="submit" className="w-full rounded-full py-6 text-base font-semibold bg-navy hover:bg-navy/90 hover:shadow-[0_8px_20px_-6px_rgba(10,37,64,0.3)] transition-all duration-300" disabled={pending}>
            {pending ? "..." : t("signIn")}
          </Button>
        </div>
      </form>
      ...
      <div className="relative my-8">
        ...
        <div className="relative flex justify-center text-xs">
          <span className="bg-white px-4 font-medium uppercase tracking-wider text-slate-400">{t("dividerText")}</span>
        </div>
      </div>
